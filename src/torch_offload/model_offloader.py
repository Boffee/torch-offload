...
class ModelOffloader:
    ...
    def optimizer_step(self) -> Iterator[None]:
        ...
        *CPU* instead — keeping its state on the host — call
        ...
        context: the trainable ``.data`` is then its pinned CPU storage and
        ``.grad`` lives on CPU, so the step runs on CPU and the in-place
        update is streamed on the next forward. Keep such trainables in fp32
        so the update is a correct master-weight update::
